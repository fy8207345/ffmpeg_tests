...
static int open_input_file(const char *filename){
    ...
    ret = av_find_best_stream(input, AVMEDIA_TYPE_VIDEO, -1, -1, &codec, 0);
    if (ret < 0) {
    ...
    avcodec_parameters_to_context(decoderContext, input->streams[videoStreamIndex]->codecpar);

    /* init the video decoder */
    if ((ret = avcodec_open2(decoderContext, codec, NULL)) < 0) {
...
static int init_filters(const char *filters_descr){
    ...
    AVRational time_base = input->streams[videoStreamIndex]->time_base;
    ...
    ret = avfilter_graph_create_filter(&sinkContext, sink, "out",
                                       NULL, NULL, filterGraph);
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Cannot create buffer sink\n");
        goto end;
    }
    ret = av_opt_set_int_list(sinkContext, "pix_fmts", pix_fmts, AV_PIX_FMT_NONE, AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
...
void display_frame(AVFrame *frame, AVRational timebase) {
    int x, y;
    uint8_t *p0, *p;
    int64_t delay;
    ...
    if(frame->pts != AV_NOPTS_VALUE){
        if(last_pts != AV_NOPTS_VALUE){
            ...
            delay = av_rescale_q(frame->pts - last_pts, timebase, AV_TIME_BASE_Q);
    ...
    while (1){
        ...
        if(packet.stream_index == videoStreamIndex){
            ...
            while (ret >= 0){
                ...
                if(av_buffersrc_add_frame_flags(srcContext, frame, AV_BUFFERSRC_FLAG_KEEP_REF) < 0){
                ...
                while (1){
                    ret = av_buffersink_get_frame(sinkContext, filt_frame);
                    if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
                        break;
                    if (ret < 0)
                        goto end;
                    display_frame(filt_frame, sinkContext->inputs[0]->time_base);
